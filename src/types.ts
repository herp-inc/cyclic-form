 ...
 */
export type View<T, Options extends FieldOptions<any> = FieldOptions<string>> = (input: ViewInput<T, Options>, metaData: MetaData) => VNode;
 ...
 */
export type FormDeclaration<Values extends any> = { [FieldName in keyof Values]: FieldDeclaration<Values[FieldName], { error: any; }> };

/**
 * Field implementations for the given `FormDeclaration`.
 */
export type FieldsFor<Form extends FormDeclaration<any>> = { [FieldName in keyof Form]?: Field<Form[FieldName]['type'], FieldOptions<Form[FieldName]['error']>> };

/**
 * Validators implementations for the given `FormDeclaration`.
 */
export type ValidatorsFor<Form extends FormDeclaration<any>> = {
    [FieldName in keyof Form]?: Form[FieldName]['error'] extends infer Err ? Validator<Form[FieldName]['type'], Err> : never;
};
 ...
 */
export type ZoomIn<Parent extends object, Scope extends Lens<Parent, any> | keyof Parent> =
    Scope extends Lens<Parent, infer Child> ? Child :
    Scope extends keyof Parent ? Parent[Scope] :
    never;
